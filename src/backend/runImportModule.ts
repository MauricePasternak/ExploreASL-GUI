...
export async function handleRunImportModule(
	...
	respondToIPCRenderer(
		event,
		`${channelName}:childProcessSTDOUT`,
		`Removed previous Import Module locked directories in ${StudyDerivExploreASLDir.path}`,
		{ bold: true }
	);

	/***************************************
	 * Step 6: Run the ExploreASL executable
	 **************************************/
	...
	child.on("spawn", () => {
		...
		respondToIPCRenderer(event, `${channelName}:childProcessHasSpawned`, child.pid, false); // false = don't clear text
