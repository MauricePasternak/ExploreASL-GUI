...
export type BIDSEnumFieldsNameType = typeof BIDSEnumFields[number];

// ^ Type Guard
export function isBIDSEnumField(fieldName: string): fieldName is BIDSEnumFieldsNameType {
	return BIDSEnumFieldsSet.has(fieldName as BIDSEnumFieldsNameType);
}

// ^ Column Definitions
type BIDSEnumBaseColDef<TName extends BIDSEnumFieldsNameType = BIDSEnumFieldsNameType, TDefault = BIDSRow[TName]> = {
	field: TName; // field name must be present for compatibility with GridColDef
	type: "singleSelect";
	defaultValue?: TDefault;
	BIDSType: "Enum";
	enumOptions: { value: TDefault; label: string }[];
};

export type BIDSEnumColDef<
	TRow extends BIDSRow = BIDSRow,
	TName extends BIDSEnumFieldsNameType = BIDSEnumFieldsNameType
> = Omit<GridColDef<TRow>, "field" | "type"> & BIDSEnumBaseColDef<TName>;

// ^ Enum Value2Label Mappings
export const BIDSEnumValue2Label: Record<BIDSEnumFieldsNameType, Record<string, string>> = {
	ArterialSpinLabelingType: {
		CASL: "CASL",
		PCASL: "PCASL",
		PASL: "PASL",
...
function BIDSEnumV2LMappingToArray(mapping: Record<string, string>) {
	return Object.entries(mapping).map(([value, label]) => ({ value, label }));
}

function BIDSEnumValueFormatter(params: GridValueFormatterParams<any>) {
	if (!params.value) return "";
	return BIDSEnumValue2Label[params.field as BIDSEnumFieldsNameType][params.value];
}

// ^ Column Definitions Map
export type BIDSEnumFieldToColDefType<
	TName extends BIDSEnumFieldsNameType = BIDSEnumFieldsNameType,
	TRow extends BIDSRow = BIDSRow
> = {
	[K in TName]: BIDSEnumColDef<TRow, K>;
};

export const BIDSEnumFieldToColDef: BIDSEnumFieldToColDefType = {
	ArterialSpinLabelingType: {
	...
	BolusCutOffTechnique: {
		...
		valueSetter: (params) => {
			...
			if (newValue === "Q2TIPS" && typeof BolusCutOffDelayTime === "number") {
				...
				return { ...oldRow, BolusCutOffTechnique: newValue, BolusCutOffDelayTime: newDelayTime };
			}
	...
	CASLType: {
		...
		width: 120,
		description:
			`For CASL sequences, describes the nature of the coil used during data acquisition.` +
	...
	M0Type: {
		...
		width: 110,
		description:
			`Describes the type of M0 image associated with this scan. ` +
	...
	PCASLType: {
		...
		width: 120,
		description:
			`For PCASL sequences, describes the type of gradient pulses used in the "control" condition. ` +
