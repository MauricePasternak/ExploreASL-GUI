...
>({
  ...
  const handleDrop = async (e: React.DragEvent<HTMLDivElement | HTMLLIElement>) => {
    ...
    const finalItemset = overWriteOnDrop ? filteredList : [...field.value, ...filteredList];
  ...
  /** Handler for choosing to delete a particular item within field.value */
  const handleDelete = (index: number) => {
    const newValue = field.value.filter((_: unknown, i: number) => i !== index);
      ...
      >
        ...
        >
          ...
          <Button
            ...
            {...clearButtonProps}
            onClick={() => {
              field.value.length > 0 && handleChange([]);
            }}
          >
            {clearButtonText}
          </Button>
