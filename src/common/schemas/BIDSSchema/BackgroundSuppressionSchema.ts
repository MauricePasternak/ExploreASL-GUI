...
export const Schema_BackgroundSuppressionPulseTime = SchemaMin_BackgroundSuppressionPulseTime.test(
	"BackgroundSuppressionPulseTime",
	"Invalid number of suppression timings",
	(bSupTimings, context) => {
		const bSupNumberPulses: number | undefined = context.parent.BackgroundSuppressionNumberPulses;
		...
		if (lodashIsUndefined(bSupNumberPulses) || bSupNumberPulses === 0) {
			if (lodashIsUndefined(bSupTimings) || (Array.isArray(bSupTimings) && bSupTimings.length === 0)) return true;
			return yupCreateError(
				context,
				`This must be an empty field when "Number of Background Suppression Pulses" is blank or zero`
			);
		}
		...
		if (lodashIsUndefined(bSupTimings) && bSupNumberPulses > 0) {
			return yupCreateError(
				context,
				`This field cannot be blank when the "Number of Background Suppression Pulses" is greater than zero`
			);
		}
		...
		if (bSupNumberPulses > 0 && bSupTimings.length !== bSupNumberPulses) {
			return yupCreateError(
				context,
				`The number of timings (${bSupTimings.length}) did not match the value in Number of Background Suppression Pulses (${bSupNumberPulses})`
			);
		}
