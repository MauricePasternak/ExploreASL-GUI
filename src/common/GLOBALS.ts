...
export const REGEXLOCKDIR = new Regex(
	"lock\\/xASL_module_(?<Module>ASL|Structural|Population|DARTEL|LongReg)\\/?(?<Subject>.*)?\\/xASL_module_(?:ASL|Structural|Population|DARTEL|LongReg)_?(?<Session>.*)\\/locked$",
	"m"
);
export const REGEXSTATUSFULE = new Regex(
	"lock\\/xASL_module_(?<Module>ASL|Structural|Population|DARTEL|LongReg)\\/?(?<Subject>.*)?\\/xASL_module_(?:ASL|Structural|Population|DARTEL|LongReg)_?(?<Session>.*)\\/(?<StatusBasename>(?<StatusCode>\\d{3}).*\\.status)$",
	"m"
);
export const REGEXIMAGEFILE = new Regex(
	"(?<Axis>Cor|Tra)_(?<ImageType>Reg_rT1|Seg_rT1|Reg_noSmooth_M0|noSmooth_M0|Reg_qCBF|qCBF)(?!.*Contour)_(?<Target>[\\w-]+?_\\d?)_?.*\\.jpe?g$",
	"m"
);
