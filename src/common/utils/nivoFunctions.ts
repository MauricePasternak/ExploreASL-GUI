import { memoize, range as lodashRange, range, round } from "lodash";
import { NivoHeatmapData, NivoHeatmapRow, NivoHeatmapSingleCell } from "../types/nivoTypes";
import { arithmeticArray, matprint, matrixify } from "./arrayFunctions";
 ...
 */
export function niftiToNivo(
  niftiData: number[],
  differenceSet: Set<number>,
  nSlices: number,
  initialIndexValues: number[],
  nCols: number,
  nRows: number,
  addValue: number,
  ...
  for (let sliceIndex = 0; sliceIndex < nSlices; sliceIndex++) {
    ...
    let elementIndex = 0;
    const nivoSliceDataRows: NivoHeatmapData = lodashRange(nRows).map(rowIdx => {
      // Prepare each row
      const rowData: NivoHeatmapRow = { id: rowIdx.toString(), data: [] as NivoHeatmapSingleCell[] };
...
export function niftiToNivoAxialAlt(niftiData: number[]) {
  const nCols = 121;
  const nRows = 145;
  const nSlices = 121;
  ...
  const nivoAxial = range(nSlices).map(sliceIndex => {
    ...
    let elementIndex = 0;
    const sliceData: NivoHeatmapData = range(nRows).map(rowIdx => {
      const rowData: NivoHeatmapRow = { id: rowIdx.toString(), data: [] as NivoHeatmapSingleCell[] };
      for (let colIdx = 0; colIdx < nCols; colIdx++) {
 ...
 */
export function niftiToNivoAxial(niftiData: number[]) {
  const nCols = 121;
  const nRows = 145;
  const nSlices = 121;
  const firstValuesOfEachSlice = arithmeticArray(17544, 17545, 121);
  ...
  for (let sliceIndex = 0; sliceIndex < nSlices; sliceIndex++) {
    ...
    const nivoSliceDataRows: NivoHeatmapData = range(nRows).map(rowIdx => {
      // Prepare each row
      const rowData: NivoHeatmapRow = { id: rowIdx.toString(), data: [] as NivoHeatmapSingleCell[] };
  ...
  return [data, nanMin, nanMax] as const;
}
...
export function niftiToNivoCoronalAlt(niftiData: number[]) {
  const nCols = 121;
  const nRows = 121;
  const nSlices = 145;
...
export function niftiToNivoCoronal(niftiData: number[]) {
  const nCols = 121;
  const nRows = 121;
  const nSlices = 145;
  const subtractionIndices = new Set(arithmeticArray(118, 121, 120));
...
export function niftiToNivoSagittalAlt(niftiData: number[]) {
  const nCols = 145;
  const nRows = 121;
  const nSlices = 121;
...
export function niftiToNivoSagittal(niftiData: number[]) {
  const nCols = 145;
  const nRows = 121;
  const nSlices = 121;
  const firstValuesOfEachSlice = arithmeticArray(2122824, 1, 121);
