	...
	const parseSingleJSON = async (jsonPath: string, basename: string, jsonIndex: number) => {
		...
		const data = Object.entries(payload).reduce(
			(acc, [key, value]) => {
				...
				acc[key as BIDSFieldNamesType] = value;
				return acc;
			},
			{ ID: jsonIndex, Filename: basename, File: jsonPath } as Record<BIDSColumnName, unknown>
		);
...
export const atomRDGColumnConfigs = atom<Column<BIDSRow>[]>((get) => {
	...
	const columns = colNames
		.map((colName) => {
			if (colName === "ID" || colName === "Filename") {
				return {
					key: colName,
					name: colName,
					editable: false,
				...
				// NUMERICAL columns
				// TODO: Alter the numerical fields to accomodate array types like `BolusCutOffDelayTime`
