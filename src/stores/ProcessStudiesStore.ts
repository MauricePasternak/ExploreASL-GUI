...
import {
  ProcessStudiesTabOption,
  RunEASLModuleNamesType,
  RunEASLStudySetupType,
} from "../common/types/ProcessStudiesTypes";
 ...
 */
export const atomProcStudyCurrentTab = atom<ProcessStudiesTabOption>("RunExploreASL");

export const atomProcStudyPIDs = atom<number[]>([]);
 ...
 */
export const atomProcStudySetups = atom<RunEASLStudySetupType[]>([
  {
    studyRootPath: "",
    numberOfCores: 1,
 ...
 */
export const atomNumCoresByAllStudies = atom(get =>
  get(atomProcStudySetups).reduce((acc, studyParams) => {
 ...
 */
export const atomAddOrRemoveStudy = atom(null, (get, set, nStudiesSelection: number) => {
  const oldStudies = get(atomProcStudySetups);
  const delta = nStudiesSelection - oldStudies.length;

  let newStudies: RunEASLStudySetupType[] = [];
  // Adding new studies
  if (delta > 0) {
    newStudies = [
      ...oldStudies,
      ...lodashRange(delta).map(() => ({
        studyRootPath: "",
        numberOfCores: 1,
        currentStatus: "Standby" as const,
        whichModulesToRun: "Structural" as const,
      })),
    ];
  } else {
    // Removing studies
 ...
 */
export const atomSetNumCoresForAStudy = atom(
  null,
  (get, set, updater: { studyIndex: number; numberOfCores: number }) => {
    const newStudies = get(atomProcStudySetups).map((studySetup, studyIndex) => {
      return updater.studyIndex !== studyIndex ? studySetup : { ...studySetup, numberOfCores: updater.numberOfCores };
    });
    set(atomProcStudySetups, newStudies);
  }
);
 ...
 */
export const atomSetModuleToRunForAStudy = atom(
  null,
  (get, set, updater: { studyIndex: number; moduleToRun: RunEASLModuleNamesType }) => {
    const newStudies = get(atomProcStudySetups).map((studySetup, studyIndex) => {
      return updater.studyIndex !== studyIndex ? studySetup : { ...studySetup, whichModulesToRun: updater.moduleToRun };
    });
    set(atomProcStudySetups, newStudies);
  }
);
 ...
 */
export const atomSetStatusForAStudy = atom(
  null,
  (get, set, updater: { studyIndex: number; status: RunEASLStatusType }) => {
    const newStudies = get(atomProcStudySetups).map((studySetup, studyIndex) => {
      return updater.studyIndex !== studyIndex ? studySetup : { ...studySetup, currentStatus: updater.status };
    });
    set(atomProcStudySetups, newStudies);
  }
);
 ...
 */
export const atomSetPathForAStudy = atom(
  null,
  (get, set, updater: { studyIndex: number; newStudyRootPath: string }) => {
    const newStudies = get(atomProcStudySetups).map((studySetup, studyIndex) => {
      return updater.studyIndex !== studyIndex
        ? studySetup
        : { ...studySetup, studyRootPath: updater.newStudyRootPath };
    });
    set(atomProcStudySetups, newStudies);
  }
);
