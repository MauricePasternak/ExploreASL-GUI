 ...
 */
function getTokenOrdering(folderStructure) {
    const filteredStructure = folderStructure.filter((folder) => folder !== "Ignore");
    const tokenOrdering = ["Subject", "Visit", "Session", "Scan"].map((v) => filteredStructure.indexOf(v) + 1 // +1 to account for MATLAB indexing
    );
    return tokenOrdering;
}
/**
 ...
 */
export function getAliasBasenames(datasetRootDir, folderStructure) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (scanDepth === -1)
                return false; // Scans must be present; something went wrong otherwise.
            // Next get the paths concurrently
            const [scanPaths, visitPaths, sessionPaths] = yield Promise.all([scanDepth, visitDepth, sessionDepth].map((depth) => {
 ...
 */
function getASLContext({ ASLSeriesPattern, NVolumes, M0PositionInASL, DummyPositionInASL, }) {
    return __awaiter(this, void 0, void 0, function* () {
...
export function buildSourceStructureJSON(importSchema) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const subjectNames = importSchema.ImportContexts.flatMap((context) => context.Paths.map((path) => {
            ...
            const filteredScanMapping = lodashPickBy(importSchema.MappingScanAliases, (alias) => alias && !["Ignore", ""].includes(alias));
...
export function buildStudyParJSON(importSchema) {
    return __awaiter(this, void 0, void 0, function* () {
        console.log("⚙️ buildStudyParJSON called with importSchema:", importSchema);
        try {
            const result = [];
            for (const context of importSchema.ImportContexts) {
                const { ASLSeriesPattern, NVolumes, M0PositionInASL, SubjectRegExp, VisitRegExp, SessionRegExp, DummyPositionInASL, } = context;
                const ASLContext = yield getASLContext({ ASLSeriesPattern, NVolumes, M0PositionInASL, DummyPositionInASL });
                ...
                const temp = {
                    ...
                    // ASL Info
                    ArterialSpinLabelingType: context.ArterialSpinLabelingType === "CASL" ? "PCASL" : context.ArterialSpinLabelingType,
 ...
 * context.
 */
export function updateContextSpecificRegexps(importSchema) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const pathSourcedata = `${makeForwardSlashes(importSchema.StudyRootPath)}/sourcedata/`;
            const newContexts = importSchema.ImportContexts.flatMap((context) => expandImportContext(context, pathSourcedata, // We need to pass the path to the sourcedata folder to the function as a forward slash!
            importSchema.SourcedataStructure, importSchema.MappingVisitAliases, importSchema.MappingSessionAliases));
 ...
 */
export function expandImportContext(singleContext, pathSourcedata, folderStructure, MappingVisitAliases, MappingSessionAliases) {
    //? An "infoType" being a concatentation of the types of info in a path, e.g. "SubjectVisitSession"
    const infoTypeToPathsMapping = {};
    ...
    for (const path of singleContext.Paths) {
        ...
        const pathInfo = new Set(pathParts
            .map((_, index) => {
    ...
    const newContexts = ObjectEntries(infoTypeToPathsMapping).flatMap(([_, infoContainer]) => {
        const { Paths, SubjectRegExp, VisitRegExp, SessionRegExp } = singleContext, restOfContext = __rest(singleContext, ["Paths", "SubjectRegExp", "VisitRegExp", "SessionRegExp"]);
        // Regexps need to be converted to strings
        const newSubjectRegExp = infoContainer.subjectRegExpList.length > 0
            ? stringArrToRegex(lodashUniq(infoContainer.subjectRegExpList), {
                isCaptureGroup: false,
                isStartEndBound: false,
            })
            : "";
        const newVisitRegExp = infoContainer.visitRegExpList.length > 0
            ? stringArrToRegex(lodashUniq(infoContainer.visitRegExpList), { isCaptureGroup: false, isStartEndBound: false })
            : "";
        const newSessionRegExp = infoContainer.sessionRegExpList.length > 0
            ? stringArrToRegex(lodashUniq(infoContainer.sessionRegExpList), {
                isCaptureGroup: false,
                isStartEndBound: false,
            })
            : "";
        const newContext = Object.assign({ Paths: infoContainer.paths, SubjectRegExp: newSubjectRegExp, VisitRegExp: newVisitRegExp, SessionRegExp: newSessionRegExp }, restOfContext);
        return newContext;
    });
    return newContexts;
}
 ...
 */
export function fetchGUIImportPar(studyRootPath) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const importParPath = api.path.asPath(studyRootPath, GUIIMPORTFILE_BASENAME);
            // If the file doesn't exist, return false
            if (!(yield api.path.filepathExists(importParPath.path)))
                return false;
            // If the file exists, try to read it
            const { payload, error } = yield api.path.readJSONSafe(importParPath.path);
            if (error)
                return false;
            // Validate the data
            const { errors, values } = yield YupValidate(SchemaImportPar, payload);
...
//# sourceMappingURL=ImportModuleHelperFunctions.js.map