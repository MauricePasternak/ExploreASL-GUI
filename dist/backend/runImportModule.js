...
import { createRuntimeEnvironment, getExploreASLVersion, getMATLABPathAndVersion, } from "./runExploreASLHelperFunctions";
// import { buildSourceStructureJSON, buildStudyParJSON } from "./runImportModuleHelperFunctions";
export function handleRunImportModule(event, channelName, formData) {
    var _a, e_1, _b, _c;
    return __awaiter(this, void 0, void 0, function* () {
         ...
         *****************************************/
        const { EASLVersionPath, EASLVersionNumber } = yield getExploreASLVersion(ExploreASLPath.path);
         ...
         **********************************************/
        const { matlabPath, matlabVersion, message: getMatlabMessage } = yield getMATLABPathAndVersion();
         ...
         ****************************************/
        const EASLEnv = yield createRuntimeEnvironment(formData.EASLType, formData.EASLPath, formData.MATLABRuntimePath);
        ...
        if (EASLVersionNumber < 110) {
            (yield rootBIDS2LegacyLockDir.exists()) && (yield rootBIDS2LegacyLockDir.remove());
            (yield rootImportLockDir.exists()) && (yield rootImportLockDir.remove());
        }
        ...
        const nWorkers = 1;
        if (formData.EASLType === "Github") {
            console.log(`EASL Process ${channelName} -- commandline input:\n`, `${executablePath}`, ...MATLABGithubArgs, `ExploreASL('${formData.StudyRootPath}', ${runImport}, ${runEASLModules}, ${bPause}, ${iWorker}, ${nWorkers})`);
            ...
            console.log(`EASL Process ${channelName} -- commandline input:\n`, `${executablePath}`, formData.MATLABRuntimePath, formData.StudyRootPath, `${runImport}`, `${runEASLModules}`, `${bPause} ${iWorker} ${nWorkers}`);
            child = spawn(executablePath, [
...
//# sourceMappingURL=runImportModule.js.map