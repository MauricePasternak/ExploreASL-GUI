...
function linkSanitize(url, protocols) {
    const anchor = document.createElement("a");
    anchor.href = url;
    const protocol = anchor.href.slice(0, anchor.href.indexOf(":"));
    return protocols.indexOf(protocol) > -1;
}
function defaultImageSanitize(url) {
 ...
 */
export const IPCQuill = React.memo((_a) => {
    var { channelName, defaultHeight = 530, scrollDownPerUpdate = false } = _a, paperProps = __rest(_a, ["channelName", "defaultHeight", "scrollDownPerUpdate"]);
    const { api } = window;
     ...
     */
    const handleIncomingText = (text, format) => {
    ...
    const wrapperRef = useCallback((wrapper) => {
        ...
        Image.sanitize = function (url) {
            ...
            if (new RegExp(String.raw `^${MEDIAPROTOCOL}`).test(url))
                return url;
    ...
    return (React.createElement(Paper, Object.assign({ elevation: 2, sx: { height: defaultHeight, overflowY: "auto", padding: 0.5, borderRadius: 2 } }, paperProps, { className: "IPCQuillContainer", ref: wrapperRef })));
}, FastIsEqual);
//# sourceMappingURL=IPCQuill.js.map