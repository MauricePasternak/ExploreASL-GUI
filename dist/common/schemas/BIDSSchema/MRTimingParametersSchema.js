...
export const SchemaMin_EchoTime = Yup.mixed().test("FitsEchoTimeType", "Echo Time must be either a positive number or an array of positive numbers", (echoTime) => {
    // Permit undefined or single-numeric values
    if (lodashIsUndefined(echoTime) || typeof echoTime === "number")
        return true;
...
export const Schema_EchoTime = SchemaMin_EchoTime.required("Echo Time is a required field in ASL processing").test("EchoTimeBIDSValid", "Echo Time must be a positive number or an array of positive numbers", (echoTime, context) => {
    ...
    // Otherwise, must be an array of numbers
    if (!Array.isArray(echoTime))
        return false;
    if (echoTime.length === 0) {
        return yupCreateError(context, "If provided as a collection of numbers, Echo Time must contain at least one positive number");
    }
...
export const Schema_RepetitionTimePreparation = SchemaMin_RepetitionTimePreparation.required("Repetition Time is a required field in ASL processing");
...
export const SchemaMin_SliceEncodingDirection = Yup.string()
    .optional()
    .oneOf(["i", "j", "k", "i-", "j-", "k-"], `Slice Encoding Direction must be one of the following: "i", "j", "k", "i-", "j-", "k-"`)
...
export const SchemaMin_PhaseEncodingDirection = Yup.string()
    ...
    .oneOf(["i", "j", "k", "i-", "j-", "k-"], `Phase Encoding Direction must be one of the following: "i", "j", "k", "i-", "j-", "k-"`);
...
//# sourceMappingURL=MRTimingParametersSchema.js.map