...
export const Schema_ArterialSpinLabelingType = SchemaMin_ArterialSpinLabelingType.required("Arterial Spin Labeling Type is a required field");
/** Minimal schema to ensure PLD is an **optional** positive number */
export const SchemaMin_PostLabelingDelay = Yup.mixed()
    .optional()
    .test("PostLabelingDelay__Lenient", "Post Labeling Delay must be either a positive number or a collection of positive numbers", (postLabelingDelay, context) => {
    if (lodashIsUndefined(postLabelingDelay) || lodashIsNumber(postLabelingDelay))
        return true;
    if (Array.isArray(postLabelingDelay) && postLabelingDelay.every((value) => lodashIsNumber(value)))
        return true;
    return false;
});
/** Expanded PLD schema to include its required status */
export const Schema_PostLabelingDelay = SchemaMin_PostLabelingDelay.required("Post Labeling Delay is a required field").test("PostLabelingDelay__Strict", "Post Labeling Delay must be either a single positive number of a collection of positive numbers", (postLabelingDelay, context) => {
    if (lodashIsNumber(postLabelingDelay)) {
        if (postLabelingDelay <= 0) {
            return yupCreateError(context, "Post Labeling Delay cannot be a non-positive number. It must be either a single positive number of a collection of such numbers (typically, separated by commas)");
        }
        return true; // Single positive number; early exit
    }
    if (!Array.isArray(postLabelingDelay))
        return yupCreateError(context, "Post Labeling Delay must be either a single positive number of a collection of such numbers (typically, separated by commas)");
...
export const SchemaMin_SliceTiming = Yup.mixed().test("SliceTiming__Lenient", "Slice Timing must be either a single positive number or a collection of such", (sliceTiming, context) => {
    if (lodashIsUndefined(sliceTiming) || lodashIsNumber(sliceTiming))
        return true;
    if (Array.isArray(sliceTiming) && sliceTiming.every((value) => lodashIsNumber(value)))
        return true;
    return false;
});
export const Schema_SliceTiming = Yup.mixed().when("MRAcquisitionType", {
    ...
    then: (schema) => schema
        ...
        .test("SliceTiming__Strict__2D", "Slice Timing must be either a single positive number or a collection of such", (sliceTiming, context) => {
        if (lodashIsNumber(sliceTiming)) {
            if (sliceTiming <= 0) {
                return yupCreateError(context, "Slice Timing cannot be a single negative number. Provide either a single positive number or a collection of incrementing non-negative numbers (separated by commas)");
            }
            return true; // Single positive number; early exit
        }
        if (!Array.isArray(sliceTiming) || sliceTiming.length === 0)
            return false;
    ...
    otherwise: (schema) => schema.test("SliceTiming__Strict__Not2D", "Slice Timing should not be provided for 3D ASL acquisitions", (sliceTiming, context) => lodashIsUndefined(sliceTiming)),
});
...
export const Schema_PCASLType = SchemaMin_PCASLType.when("ArterialSpinLabelingType", {
    is: "PCASL",
    then: (schema) => schema,
    otherwise: (schema) => schema.test("PCASLType__NotPCASL", "PCASL Type should not be provided for non-PCASL ASL acquisitions", (caslType, context) => {
        // Because this is a conditional schema, we know ASLType is not PCASL
        const ASLType = context.parent.ArterialSpinLabelingType;
        if (lodashIsUndefined(caslType))
            return true;
        if (lodashIsUndefined(ASLType)) {
            return yupCreateError(context, `PCASL Type cannot be provided without first indicating the "Arterial Spin Labeling Type"`);
        }
        else {
...
export const SchemaMin_CASLType = Yup.string()
    .oneOf(["single-coil", "double-coil"], `If provided, CASL Type must be one of the following: "Single Coil", "Double Coil"`)
    .optional()
...
export const Schema_CASLType = SchemaMin_CASLType.when("ArterialSpinLabelingType", {
    is: "CASL",
    then: (schema) => schema,
    otherwise: (schema) => schema.test("CASLType__NotCASL", "CASL Type should not be provided for non-CASL ASL acquisitions", (caslType, context) => {
        // Because this is a conditional schema, we know ASLType is not CASL
        const ASLType = context.parent.ArterialSpinLabelingType;
        if (lodashIsUndefined(caslType))
            return true;
        if (lodashIsUndefined(ASLType)) {
            return yupCreateError(context, `CASL Type cannot be provided without first indicating the "Arterial Spin Labeling Type"`);
        }
        else {
...
export const SchemaMin_LabelingDuration = Yup.mixed()
    ...
    if (lodashIsUndefined(labelingDuration) || typeof labelingDuration === "number")
        return true;
...
export const Schema_LabelingDuration = Yup.mixed()
    ...
    then: (schema) => schema
        .optional()
        .test("LabelingDuration", "Labeling Duration should not be present when Arterial Spin Labeling Type is PASL", (value) => lodashIsUndefined(value)),
    otherwise: (schema) => schema
        ...
        if (Array.isArray(labelingDuration)) {
            if (labelingDuration.length === 0) {
                return yupCreateError(context, "This field cannot be empty unless the Arterial Spin Labeling Type is PASL");
            }
...
export const SchemaMin_PASLType = Yup.string()
    .oneOf(["FAIR", "EPISTAR", "PICORE"], `If provided, PASL Type must be one of the following: "FAIR", "EPISTAR", "PICORE"`)
    .optional()
...
export const Schema_PASLType = SchemaMin_PASLType.when("ArterialSpinLabelingType", {
    is: "PASL",
    then: (schema) => schema,
    otherwise: (schema) => schema.test("PASLType__NotPASL", "PASL Type should not be provided for non-PASL ASL acquisitions", (caslType, context) => {
        // Because this is a conditional schema, we know ASLType is not PASL
        const ASLType = context.parent.ArterialSpinLabelingType;
        if (lodashIsUndefined(caslType))
            return true;
        if (lodashIsUndefined(ASLType)) {
            return yupCreateError(context, `PASL Type cannot be provided without first indicating the "Arterial Spin Labeling Type"`);
        }
        else {
...
export const Schema_BolusCutOffFlag = Yup.boolean().when("ArterialSpinLabelingType", {
    ...
    otherwise: SchemaMin_BolusCutOffFlag.test("BolusCutOffFlag", "Bolus Cut Off Flag should not be present when Arterial Spin Labeling Type is not PASL", (value) => lodashIsUndefined(value)),
});
...
export const Schema_BolusCutOffTechnique = Yup.string().when("BolusCutOffFlag", {
    is: true,
    then: SchemaMin_BolusCutOffTechnique.required("Bolus Cut Off Technique is a required field when Bolus Cut Off Flag is true"),
    otherwise: SchemaMin_BolusCutOffTechnique.test("BolusCutOffTechnique", "Bolus Cut Off Technique should not be present when Bolus Cut Off Flag is false", (value) => lodashIsUndefined(value)),
});
/** Minimal schema to ensure BolusCutOffDelayTime is either undefined, a number, or tuple of two numbers */
export const SchemaMin_BolusCutOffDelayTime = Yup.mixed().test("SchemaMin_BolusCutOffDelayTime", "Bolus Cut Off Delay Time must be either omitted, a non-negative number, or a collection of two non-negative numbers depending on the value of Bolus Cut Off Technique", (cutOffDelayTime) => {
    return (lodashIsUndefined(cutOffDelayTime) ||
...
export const Schema_BolusCutOffDelayTime = Yup.mixed()
    ...
    .test("BolusCutOffDelayTime", "Bolus Cut Off Delay Time must be either a number of a collection of two numbers depending on the values of Bolus Cut Off Technique", (delayTime, context) => {
    const { BolusCutOffFlag, BolusCutOffTechnique, } = context.parent;
    if (!BolusCutOffFlag || lodashIsUndefined(BolusCutOffTechnique)) {
        if (!!delayTime) {
            return yupCreateError(context, `Bolus Cut Off Delay Time should not be present when Bolus Cut Off Flag is false or Bolus Cut Off Technique is undefined`);
        }
        return true;
    }
    if (BolusCutOffTechnique === "Q2TIPS") {
        if (!Array.isArray(delayTime) ||
            ...
            return yupCreateError(context, `Bolus Cut Off Delay Time must be an array of two numbers when Bolus Cut Off Technique is "Q2TIPS"; instead got ${delayTime}`);
        }
    }
    else {
        if (typeof delayTime !== "number" || delayTime < 0) {
            return yupCreateError(context, `Bolus Cut Off Delay Time must be a number when Bolus Cut Off Technique is "QUIPSS" or "QUIPSSII"; instead got ${delayTime}`);
        }
    }
    return true;
});
...
//# sourceMappingURL=ASLSchema.js.map